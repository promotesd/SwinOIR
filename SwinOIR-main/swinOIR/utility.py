    ...
    def __init__(self, args):
        ...
        if not args.load:
            ...
            if os.path.exists(self.dir):
                self.log = torch.load(self.get_path('psnr_log.pt'),weights_only=True)
...
def calc_psnr(sr, hr, scale, rgb_range, dataset=None, sigma=1.5, K1=0.01, K2=0.03, R=255):
...
def make_optimizer(args, target):
    ...
    class CustomOptimizer(optimizer_class):
        ...
        def load(self, load_dir, epoch=1):
            self.load_state_dict(torch.load(self.get_dir(load_dir),weights_only=True))
            if epoch > 1:
